...
(async () => {
  ...
  function footer(s, n, dark = false) {
    s.addText([{ text: 'BrainSpark ', options: { color: dark ? '8FA1B5' : MUT, bold: true } }, { text: 'Horizon', options: { color: GOLDL, bold: true } }, { text: `  ·  Technology Foresight Engine  ·  ${n}/16`, options: { color: dark ? '64788D' : '93A3B5' } }], { x: 0.55, y: H - 0.4, w: 8, h: 0.28, fontFace: 'Courier New', fontSize: 9, margin: 0 });
  }
  ...
  // ═══ 2 · CONTEXT & AGENDA ══════════════════════════════════════════════════
  {
    ...
    const ctx = [
      ['13 AI tools, one platform', 'BrainSpark already generates cost ideas, should-costs parts, reads CAD and PCBs, and tracks savings to realisation.'],
  ...
  // ═══ 5 · UNDER THE HOOD (models + workflow) ═══════════════════════════════
  {
    ...
    const flow = [
      ...
      ['Register match', 'search-term + commodity-classifier resolution against the 139 curated technologies'],
    ...
    s.addText('Stack: deterministic JavaScript engines · curated evidence register · SQLite ledger · Claude LLM (schema-forced, grounded) · PatentsView + web retrieval · React UI — locked by 326 CI tests.', { x: 0.55, y: 6.78, w: 12.2, h: 0.35, align: 'center', fontFace: 'Courier New', fontSize: 9.5, color: MUT });
    footer(s, 5);
  ...
  // ═══ 14 · TRUST ═══════════════════════════════════════════════════════════
  {
    ...
    const checks = [
      ['326 automated tests', 'Register integrity, model rules and coverage are CI-enforced — an adoption claim without evidence fails the build.'],
