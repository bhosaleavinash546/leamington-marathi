/* ============================================================================
 * model.js — 360° PCB should-cost engine (all PCB types) — 2026 edition.
 *
 * Pure functions. Bottom-up, process-routed cost build with layer count and
 * stack-up as primary drivers. Now models fine-line process (mSAP/SAP),
 * copper-foil profile, back-drilling (per-hole), via-fill, AXI, 2026 gold-price
 * finish sensitivity, market metal/energy surcharge, and expanded regions.
 * ==========================================================================*/

const byId = (arr, id) => arr.find((x) => x.id === id) || arr[0];
const clampN = (x, lo, hi) => Math.min(hi, Math.max(lo, x));

/* Lamination cycles — central stack-up driver. Sequential lamination for
 * buried/blind vias and HDI build-ups; rigid-flex adds bonding cycles. */
function laminationCycles(p, type, via) {
  if (p.layerCount <= 2) return type.id === "flex" ? 1 : 0;
  let cycles = 1;
  cycles += via.lamExtra;
  if (type.id === "anylayer") cycles += 1;        // any-layer build adds a press
...
/* Gold-price-adjusted surface-finish cost per dm². Splits the finish into a
 * fixed process part and a spot-gold-variable part (goldFrac). */
...
function computePcb(input) {
  ...
  material += area * L * foil.addDm2;                    // copper-foil profile premium
  ...
  const fine = trace.fineMult * proc.imageMult;          // fine-line imaging tech
  ...
  const surfaceFin = area * finishCostDm2(finish, p.goldPrice);
  ...
  let processing = (imaging + etch + aoi + lamination + drilling + laserDrill + backdrill +
                    viafill + msapAdder + desmearPth + plating + solderMask + silkscreen +
                    profiling + impedance) * type.complexity * rfMult;
  processing *= region.labor;
  processing *= surcharge;
  material *= surcharge;

  // ---------------- TEST & INSPECTION ----------------
  // ICT/fixtured break-even ~500–1000 units (research); use ≥2000 for fixtured.
  ...
  if (quality.axi && (via.microvia || L >= 8)) inspection += area * COEFF.axiDm2;   // AXI for buried/HDI
  if (quality.microsection) inspection += COEFF.istCoupon * area;                   // IST/CAF coupon share
  ...
  const yld = clampN(quality.yld * trace.yld * proc.yld * layerYld *
  ...
  const pMult = type.complexity * rfMult * region.labor * surcharge;
  const components = [
    { key: "material",  label: "Material (laminate, copper, prepreg)", value: material * yScale,   color: "#00e5ff" },
    { key: "imaging",   label: "Imaging + etch + AOI",                 value: (imaging + etch + aoi) * pMult * yScale, color: "#3a8bff" },
    { key: "lamination",label: "Lamination (" + lamCycles + " cycle" + (lamCycles===1?"":"s") + ")", value: lamination * pMult * yScale, color: "#a06bff" },
    { key: "drilling",  label: "Drilling + laser µvia + back-drill",   value: (drilling + laserDrill + backdrill) * pMult * yScale, color: "#ff2bd6" },
    { key: "plating",   label: "Desmear / PTH / plating / via-fill",   value: (desmearPth + plating + viafill + msapAdder) * pMult * yScale, color: "#6cff3f" },
    { key: "finish",    label: "Solder mask / finish / silk",          value: (solderMask + surfaceFin + silkscreen + profiling + impedance) * pMult * yScale, color: "#ffcf33" },
  ...
  const dfc = dfcRecommendations(p, { type, mat, via, trace, finish, quality, proc, L, area, bpp, aspect, yld, lamCycles, cuIn, cuOut });
 ...
 * Base $/dm² midpoints are the 2026-inflated China-volume FR-4 ladder. */
function benchmarkBand(p, type, L, area, quality, finish, mat, impedance, region, maxOz) {
  ...
  const cuMult = maxOz >= 3 ? (1 + 0.15 * (maxOz - 1)) : 1.0;                 // heavy-copper premium
  const regionMult = 1 + ((region ? region.labor : 1) - 1) * 0.45;           // blended region premium
  const impMult = impedance ? 1.1 : 1.0;
  const mid = perDm2 * typeMult * qualMult * finishMult * matMult * cuMult * regionMult * impMult * area;
  return { lo: mid * 0.65, mid, hi: mid * 1.6 };
}

/* DfC (Design-for-Cost) recommendations. */
function dfcRecommendations(p, c) {
  ...
  if (c.bpp <= 1)
    ...
    push("med", `Panel utilisation ${p.utilization}% is low. Improving nesting toward 80–85% directly lowers $/board.`);
  ...
  if (p.orderQty < 2000)
    push("low", `Order qty ${p.orderQty} is below the ~500–1000 ICT break-even; flying-probe test and un-amortised tooling dominate. Consolidating volume or panel-sharing cuts NRE per board.`);
