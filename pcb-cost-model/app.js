...
const NUM_FIELDS = ["boardW","boardH","orderQty","layerCount","boardThickness",
  "panelW","panelH","utilization","holeDensity","overheadPct","marginPct","goldPrice","marketSurcharge"];
const SEL_FIELDS = ["pcbType","material","copperInner","copperOuter","copperFoil","fabProcess","via","trace","finish","maskColor","quality","region"];
const CHK_FIELDS = ["impedance","backdrill","viafill","silkscreen"];
...
function render() {
  ...
  $("accuracyNote").innerHTML =
    `<b>Accuracy:</b> calibrated to <b>2026</b> $/dm² fab-price ladders, labour/region indices, gold-price
     finish sensitivity and CCL inflation. Gold at ${input.goldPrice ? "$" + Number(input.goldPrice).toLocaleString() : "$4,100"}/oz;
     market surcharge ${input.marketSurcharge}%. Treat outputs as a should-cost <b>envelope</b> (±15–20%), not a quote.
     The model runs slightly conservative on the very simplest boards.`;
  // keep AI output fresh if already generated
  if (window._aiDirty) { $("aiOutput").dataset.stale = "1"; }
}
...
function runLocalAI() {
  ...
  $("aiOutput").innerHTML = renderLocalInsights(input, r);
  switchTab("ai");
}
async function runClaudeAI() {
  const apiKey = $("aiKey").value.trim();
  const model = $("aiModel").value;
  const status = $("aiStatus"), out = $("aiOutput");
  if (!apiKey) { status.className = "ai-status err"; status.textContent = "Enter your Anthropic API key to use Claude generation (or use the built-in advisor)."; switchTab("ai"); return; }
  if ($("aiSaveKey").checked) { try { localStorage.setItem("pcb_ai_key", apiKey); } catch (e) {} }
  const input = readInputs();
  const r = computePcb(input);
  status.className = "ai-status"; status.innerHTML = `<span class="ai-spinner"></span>Asking ${model} for a should-cost review…`;
  out.innerHTML = renderLocalInsights(input, r) + `<p class="hint">↑ built-in insights shown while Claude responds…</p>`;
  switchTab("ai");
  try {
    const text = await claudeGenerate({ apiKey, model, input, r });
    out.innerHTML = mdToHtml(text);
    status.className = "ai-status"; status.textContent = `Generated by ${model}.`;
  } catch (e) {
    status.className = "ai-status err";
    let hint = e.message || String(e);
    if (/Failed to fetch|NetworkError|CORS/i.test(hint)) hint += " — the browser may be blocking the direct call (CORS/network). The built-in advisor above still works offline.";
    status.textContent = "Claude call failed: " + hint;
  }
}
...
function init() {
  ...
  $("btnClaudeAI").addEventListener("click", runClaudeAI);
