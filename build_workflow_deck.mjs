...
}
{
  ...
  const rows = [
    ...
    ['5 · DFM / DFA insights', 'Not available', 'Generated by the deterministic COST ENGINE — 36 threshold rules + 10 geometry advisors, scores by fixed arithmetic. Not by AI: the AI cannot write a score, severity or saving', 'DFM/DFA slide'],
...
// ══════════ 10a3 · THE DFM / DFA REPORT (rules, not opinions) ══════════
{
  ...
  const steps = [
    ...
    ['2 · 36 threshold rules fire', 'Fixed engineering thresholds, each with severity, saving % and a recommendation: material utilisation below 60% is critical and below 72% major; OEE below 70% critical, below 80% major; each cost bucket compared to its commodity benchmark band; operation and setup counts checked.'],
...
// source (calculator/src/engine/dfm-dfa.ts + modules/*-advisor.ts) — same
// thresholds, same savings, same order. Keep in sync if the engine changes.
// ═══════════════════════════════════════════════════════════════════════════════
FOOT = 'CostVision · DFM, DFA & idea generation — the complete rule library';

const SEV = { Critical: RED, Major: AMBER, Minor: MUTED, Opportunity: GREEN };
...
// ══════════ A1 · OVERVIEW — WHAT RUNS AFTER THE PRICE ══════════
{
  ...
  const stats = [
    ['36', 'threshold rules', '30 manufacturability (DFM) checks — 8 on every part, 22 per commodity — plus 6 assembly (DFA) checks on the cost structure.', TEAL],
    ['10', 'parameters', 'Every threshold rule reads only these ten numbers, all from the costing you just watched being built. Nothing else goes in.', BLUE],
    ...
    ['10', 'idea levers', 'Cost-reduction ideas, each with a trigger, an expected saving, a risk grade and a timeframe. Every part leaves with 5–6 or more.', GREEN],
  ];
  ...
  const steps = [
    ['1 · Read', 'The finished costing is reduced to the ten parameters — six bucket shares, operation count, OEE, material utilisation, total.'],
    ['2 · Check', 'All 36 threshold rules run — each a plain test like "tooling above 20% of part cost" — and the geometry advisors read the solid.'],
    ...
    ['5 · Suggest', 'The ten idea levers produce 5–8 concrete actions, each tagged quick win, medium or long term, with a risk grade.'],
  ];
  steps.forEach(([h, t], i) => {
    const x = 0.72 + i * 2.4;
  ...
  s.addText([
    ...
    { text: 'nowhere in the rules. Every check, score and saving on the next five slides is deterministic engine code. The optional "AI deep analysis" only writes commentary on findings the rules have already made — it cannot add, remove or re-score one.', options: { color: SLATE } },
  ...
  s.addText([
    { text: 'What the next five slides are:  ', options: { bold: true, color: '9FB6E0' } },
    { text: 'the complete rule book, transcribed from the tool’s source code — the 10 parameters, all 36 rules with exact thresholds, the 10 geometry advisors, and all 10 idea levers.', options: { color: 'FFFFFF' } },
  ], { x: 0.85, y: 6.22, w: 11.65, h: 0.6, fontFace: 'Calibri', fontSize: 12, margin: 0, valign: 'middle' });
  footer(s, ++PG);

  s.addNotes(
    'This appendix exists because of a fair question: when the tool scores a part eight and a half out of ten for manufacturability and offers six cost-reduction ideas — where do those actually come from? ' +
    'The answer is a written rule book, and it is small enough to show you in full. Thirty-six threshold rules reading ten parameters, ten geometry advisors reading the measured solid, and ten idea levers. That is the entire analysis — there is no hidden model behind it. ' +
    'The flow is five steps and all of them are arithmetic. The finished costing is reduced to ten numbers. Thirty-six written checks run against those ten numbers, and each one either fires or passes — "is tooling more than twenty percent of the part cost, yes or no" — while the geometry advisors do the same job on the measured shape. Findings knock points off two scores that both start at ten. The three biggest savings are combined root-sum-square so we never double-count overlapping fixes. And the idea levers turn what fired into five to eight concrete actions, each tagged with risk and timeframe. ' +
    'One thing I want to be precise about, because it is the theme of this whole pack: the AI is nowhere in this. Every rule you are about to see is deterministic engine code — same part in, same findings out, every single time, with the network cable unplugged. There is an optional button that asks the AI to write commentary around the findings, but it cannot add a finding, remove one, or change a score. ' +
    'The next five slides are the complete rule book, transcribed from the source code. Nothing summarised, nothing left out.'
  );
}

// ══════════ A2 · THE 10 PARAMETERS ══════════
{
  const s = pres.addSlide(); s.background = { color: PAGE };
  title(s, 'The 10 Parameters the Rules Read', 'The complete list — everything the threshold rules are allowed to look at; the last three are typed in with the part', TEAL);
  owner(s, 10.33, 0.28, 'OWNER: THE ENGINE', TEAL, TEAL_T);

  const params = [
    ['1', 'Total part cost', 'The engine’s finished ex-works cost — the reference every percentage below is measured against.', '—'],
  ...
  s.addText('PARAMETER', { x: cx[1], y: 1.26, w: cw[1], h: 0.22, fontFace: 'Calibri', fontSize: 8.5, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('IN PLAIN WORDS', { x: cx[2], y: 1.26, w: cw[2], h: 0.22, fontFace: 'Calibri', fontSize: 8.5, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('BENCHMARK', { x: cx[3], y: 1.26, w: cw[3], h: 0.22, fontFace: 'Calibri', fontSize: 8.5, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  params.forEach((r, i) => {
    const y = 1.52 + i * 0.51;
    s.addShape('roundRect', { x: 0.5, y, w: 9.75, h: 0.46, fill: { color: CARD }, line: { color: LINE, width: 1 }, rectRadius: 0.07 });
    s.addShape('ellipse', { x: cx[0], y: y + 0.09, w: 0.28, h: 0.28, fill: { color: i < 7 ? TEAL : BLUE } });
    s.addText(r[0], { x: cx[0], y: y + 0.09, w: 0.28, h: 0.28, fontFace: 'Calibri', fontSize: 9.5, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', margin: 0 });
    s.addText(r[1], { x: cx[1], y, w: cw[1], h: 0.46, fontFace: 'Calibri', fontSize: 10.5, bold: true, color: NAVY, margin: 0, valign: 'middle' });
    s.addText(r[2], { x: cx[2], y, w: cw[2], h: 0.46, fontFace: 'Calibri', fontSize: 9.2, color: SLATE, margin: 0, valign: 'middle' });
    s.addText(r[3], { x: cx[3], y, w: cw[3], h: 0.46, fontFace: 'Calibri', fontSize: 8.8, italic: true, color: MUTED, margin: 0, valign: 'middle' });
  });
  s.addText('Teal — read straight off the finished costing.   Blue — entered with the part (with a stated assumption used when left blank).',
    { x: 0.5, y: 6.66, w: 9.75, h: 0.24, fontFace: 'Calibri', fontSize: 8.5, italic: true, color: MUTED, margin: 0 });

  // scoring card
  s.addShape('roundRect', { x: 10.45, y: 1.52, w: 2.38, h: 5.06, fill: { color: CARD }, line: { color: TEAL, width: 1.5 }, rectRadius: 0.1 });
  s.addText('HOW THE SCORE\nIS BUILT', { x: 10.62, y: 1.66, w: 2.05, h: 0.55, fontFace: 'Calibri', fontSize: 10, bold: true, color: TEAL, charSpacing: 0.4, margin: 0 });
  s.addText([
    { text: 'Start at 10.\n', options: { bold: true, color: NAVY } },
    ...
    { text: 'the three biggest findings combined root-sum-square — not added — and capped at 40%, so overlapping fixes are not double-counted.', options: { color: SLATE } },
  ], { x: 10.62, y: 2.28, w: 2.05, h: 4.2, fontFace: 'Calibri', fontSize: 9, margin: 0, valign: 'top' });

  footer(s, ++PG);
  s.addNotes(
    'These ten numbers are the complete diet of the threshold rules — a rule is not allowed to read anything else. ' +
    'Seven of them, in teal, come straight off the costing you watched being built: the total, and the share of that total sitting in each of six buckets — material, process, labour, tooling, overhead and margin. ' +
    'Three of them, in blue, arrive with the part: how many process steps it goes through, the OEE of the machines making it, and how much of the bought material ends up in the finished part. If nobody enters the last two, the tool uses a stated assumption — eighty-five percent OEE, seventy-two percent utilisation — and it says so rather than hiding it. ' +
    'Each parameter carries a benchmark, and the benchmarks are where the rules get their teeth: tooling wants to be near twelve percent of part cost, overhead under eighteen, margin ten to twelve, OEE at eighty-five. ' +
    'And on the right, the whole of the scoring system — small enough to fit on a card. Start at ten. Two points off per critical finding, one per major, half per minor; opportunities are free. Do that once for manufacturability and once for assembly and you have the two scores. The headline saving takes the three biggest findings and combines them root-sum-square, capped at forty percent — deliberately not a straight sum, because two fixes that overlap do not save you twice.'
  );
}

// ══════════ A3 · RULE LIBRARY 1/2 — THE 14 UNIVERSAL CHECKS ══════════
{
  const s = pres.addSlide(); s.background = { color: PAGE };
  title(s, 'Rule Library 1 of 2 — 14 Checks on Every Part', '8 manufacturability rules on any commodity, then 6 assembly rules · thresholds exactly as coded', TEAL);
  owner(s, 10.33, 0.28, 'OWNER: THE ENGINE', TEAL, TEAL_T);

  const rcx = [0.72, 3.85, 9.6, 11.9], rcw = [3.05, 5.65, 2.2, 0.75];
  const header = y => {
    s.addText('RULE', { x: rcx[0], y, w: rcw[0], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
    s.addText('FIRES WHEN', { x: rcx[1], y, w: rcw[1], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
    s.addText('SEVERITY', { x: rcx[2], y, w: rcw[2], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
    s.addText('SAVE', { x: rcx[3], y, w: rcw[3], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, align: 'right', charSpacing: 0.5, margin: 0 });
  };
  const row = (y, n, [rule, when, sev, save]) => {
    if (n % 2 === 0) s.addShape('rect', { x: 0.5, y: y - 0.015, w: 12.33, h: 0.325, fill: { color: CARD } });
    s.addText(rule, { x: rcx[0], y, w: rcw[0], h: 0.3, fontFace: 'Calibri', fontSize: 9.2, bold: true, color: NAVY, margin: 0, valign: 'middle' });
    s.addText(when, { x: rcx[1], y, w: rcw[1], h: 0.3, fontFace: 'Calibri', fontSize: 9, color: SLATE, margin: 0, valign: 'middle' });
    s.addText(sev, { x: rcx[2], y, w: rcw[2], h: 0.3, fontFace: 'Calibri', fontSize: 9, bold: true, color: SEV[sev], margin: 0, valign: 'middle' });
    s.addText(save, { x: rcx[3], y, w: rcw[3], h: 0.3, fontFace: 'Calibri', fontSize: 9.2, bold: true, color: TEAL, align: 'right', margin: 0, valign: 'middle' });
  };

  s.addShape('roundRect', { x: 0.5, y: 1.16, w: 12.33, h: 0.28, fill: { color: TEAL_T }, rectRadius: 0.06 });
  s.addText('DFM — MANUFACTURABILITY · 8 RULES, ANY COMMODITY', { x: 0.72, y: 1.16, w: 11.9, h: 0.28, fontFace: 'Calibri', fontSize: 9, bold: true, color: TEAL, charSpacing: 0.6, margin: 0, valign: 'middle' });
  header(1.46);
  const dfm = [
  ...
  dfm.forEach((r, i) => row(1.70 + i * 0.325, i, r));

  s.addShape('roundRect', { x: 0.5, y: 4.40, w: 12.33, h: 0.28, fill: { color: GREEN_T }, rectRadius: 0.06 });
  s.addText('DFA — ASSEMBLY · 6 RULES', { x: 0.72, y: 4.40, w: 11.9, h: 0.28, fontFace: 'Calibri', fontSize: 9, bold: true, color: GREEN, charSpacing: 0.6, margin: 0, valign: 'middle' });
  header(4.72);
  const dfa = [
    ['Multiple setups and transfers', 'More than 5 operations — each setup adds handling time and variation', 'Major', '8%'],
    ...
    ['Repeated re-fixturing', 'Machined or forged part with more than 3 operations — pallet systems cut handling', 'Minor', '5%'],
    ['Fastener standardisation', 'More than 4 operations with labour above 20% — check fastener variety', 'Opportunity', '4%'],
  ];
  dfa.forEach((r, i) => row(4.96 + i * 0.325, i, r));

  s.addText('"Save" is the rule’s own estimate of what fixing the finding is worth, as a % of part cost — the headline combines the top three (previous slide).',
    { x: 0.5, y: 6.92, w: 12.33, h: 0.22, fontFace: 'Calibri', fontSize: 8.5, italic: true, color: MUTED, margin: 0 });
  footer(s, ++PG);

  s.addNotes(
    'Here is the first half of the threshold rule book — the fourteen checks that run on every single part, whatever it is made of. ' +
    'The top eight are manufacturability. Read them as a story about waste: are you throwing away too much material — critical below sixty percent utilisation, major below the seventy-two percent benchmark. Are the machines earning their keep — critical below seventy percent OEE, major below eighty. Is the tooling bill carried by too few parts — major above twenty percent of part cost, minor above twelve. And the two commercial ones: overhead above eighteen percent, margin above eighteen percent. Those last two are pure negotiation levers. ' +
    'The bottom six are assembly. More than five operations means setups and transfers. High labour with several operations triggers an automation feasibility question. Low OEE in an assembly context usually means hand-paced work. A harness or a circuit board with labour above forty-five percent is the one critical in this group — fixture it or automate it before anything else. And the two small ones: re-fixturing on machined parts, and fastener variety. ' +
    'Two things to notice. Every threshold is a number, printed on the slide — there is no judgement call hiding anywhere. And the save column is the rule’s own estimate of what fixing it is worth; the headline saving never just adds them up, it takes the biggest three root-sum-square, as on the previous slide.'
  );
}

// ══════════ A4 · RULE LIBRARY 2/2 — THE 22 COMMODITY CHECKS ══════════
{
  const s = pres.addSlide(); s.background = { color: PAGE };
  title(s, 'Rule Library 2 of 2 — The 22 Commodity Checks', 'The rules that switch on for the process actually being costed · thresholds exactly as coded', TEAL);
  owner(s, 10.33, 0.28, 'OWNER: THE ENGINE', TEAL, TEAL_T);

  const ccx = [0.72, 2.42, 5.5, 11.0, 12.35], ccw = [1.65, 3.0, 5.4, 1.3, 0.35];
  s.addText('COMMODITY', { x: ccx[0], y: 1.26, w: ccw[0], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('CHECK', { x: ccx[1], y: 1.26, w: ccw[1], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('FIRES WHEN', { x: ccx[2], y: 1.26, w: ccw[2], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('SEVERITY', { x: ccx[3], y: 1.26, w: ccw[3], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('SAVE', { x: ccx[4], y: 1.26, w: ccw[4], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, align: 'right', charSpacing: 0.4, margin: 0 });

  const rules = [
    ['Machining', 'High operation count', 'More than 4 operations — every extra setup adds fixtures, handling and variation', 'Major', '10%'],
    ['', 'Process cost dominates', 'Machining is over 40% of part cost — consider a near-net cast or forged blank', 'Major', '8%'],
    ['Casting', 'Die cost very high', 'Die is over 18% of part cost — volume too low to carry the die investment', 'Critical', '12%'],
  ...
  rules.forEach((r, i) => {
    const y = 1.52 + i * 0.235;
    if (r[0]) s.addShape('rect', { x: 0.5, y: y - 0.01, w: 12.33, h: 0.02, fill: { color: LINE } });
    else if (i % 2 === 1) s.addShape('rect', { x: 0.5, y: y - 0.005, w: 12.33, h: 0.225, fill: { color: CARD } });
    s.addText(r[0], { x: ccx[0], y, w: ccw[0], h: 0.22, fontFace: 'Calibri', fontSize: 8.6, bold: true, color: TEAL, margin: 0, valign: 'middle' });
    s.addText(r[1], { x: ccx[1], y, w: ccw[1], h: 0.22, fontFace: 'Calibri', fontSize: 8.6, bold: true, color: NAVY, margin: 0, valign: 'middle' });
    s.addText(r[2], { x: ccx[2], y, w: ccw[2], h: 0.22, fontFace: 'Calibri', fontSize: 8.4, color: SLATE, margin: 0, valign: 'middle' });
    s.addText(r[3], { x: ccx[3], y, w: ccw[3], h: 0.22, fontFace: 'Calibri', fontSize: 8.4, bold: true, color: SEV[r[3]], margin: 0, valign: 'middle' });
    s.addText(r[4], { x: ccx[4], y, w: ccw[4], h: 0.22, fontFace: 'Calibri', fontSize: 8.4, bold: true, color: TEAL, align: 'right', margin: 0, valign: 'middle' });
  });

  s.addText('"Moulding family" covers injection, blow moulding and thermoforming. With the 14 universal checks: 36 rules in total — 30 DFM + 6 DFA.',
    { x: 0.5, y: 6.86, w: 12.33, h: 0.24, fontFace: 'Calibri', fontSize: 8.5, italic: true, color: MUTED, margin: 0 });
  footer(s, ++PG);

  s.addNotes(
    'And the second half — the twenty-two checks that switch on for the specific process being costed. A casting is never judged by sheet-metal rules; the tool applies the block that matches the commodity it just priced. ' +
    'A few worth picking out rather than reading all twenty-two. On machining: more than four operations, or machining above forty percent of the part cost — both pointing the same way, towards a near-net blank. On casting, the critical one is the die above eighteen percent of part cost — volume too low to carry the tool, which is exactly the bumper conversation from the earlier section. Sheet metal gets four rules, including one that reads the operation list itself: continuous MIG or TIG welding on thin sheet fires a distortion warning. The moulding family — injection, blow and thermoforming — shares two rules: mould above thirty percent of cost, and runner waste a hot-runner would remove. ' +
    'And notice the two biggest numbers on the slide sit with wiring harness and composites — labour past half the cost on a harness, hand layup past forty percent on a composite. Those are the two commodities where the money is overwhelmingly people’s time, and the rules say so bluntly. ' +
    'One honest footnote: the casting material-content rule carries no saving at all. It is a data-quality check — material under thirty-five percent of a casting’s cost usually means somebody entered the wrong alloy or weight, and the tool would rather query the input than congratulate you on cheap metal. ' +
    'Add the fourteen from the previous slide and that is the whole threshold rule book: thirty-six rules, thirty DFM and six DFA.'
  );
}

// ══════════ A5 · THE 10 GEOMETRY ADVISORS ══════════
{
  const s = pres.addSlide(); s.background = { color: PAGE };
  ...
  s.addText('75 geometry checks in total. They run alongside the 36 cost-structure rules and feed the same DFM/DFA sections of the report — findings the AI can comment on but never write.',
    { x: 0.5, y: 6.62, w: 12.33, h: 0.4, fontFace: 'Calibri', fontSize: 8.5, italic: true, color: MUTED, margin: 0 });
  footer(s, ++PG);

  s.addNotes(
    'The thirty-six rules you have just seen read the cost structure. This slide is the other half of the DFM story: the ten geometry advisors, which read the measured solid itself — the walls, the draft, the radii, the spans — one advisor module per manufacturing process, seventy-five checks between them. ' +
    ...
    'The important sentence is the footnote: these seventy-five checks run alongside the thirty-six cost-structure rules and feed the same sections of the report. All of it deterministic, all of it from the measured geometry — and the AI can add commentary to these findings, but it cannot write one.'
  );
}

// ══════════ A6 · THE 10 IDEA-GENERATION LEVERS ══════════
{
  const s = pres.addSlide(); s.background = { color: PAGE };
  title(s, 'Idea Generation — The 10 Optimisation Levers', 'How findings become an action list: each lever has a trigger, a saving, a risk and a timeframe', GREEN);
  owner(s, 10.33, 0.28, 'OWNER: THE ENGINE', TEAL, TEAL_T);

  const lcx = [0.72, 4.35, 8.6, 10.15, 11.35], lcw = [3.55, 4.1, 1.45, 1.1, 1.45];
  s.addText('LEVER', { x: lcx[0], y: 1.26, w: lcw[0], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('SUGGESTED WHEN', { x: lcx[1], y: 1.26, w: lcw[1], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('SAVING UP TO', { x: lcx[2], y: 1.26, w: lcw[2], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('RISK', { x: lcx[3], y: 1.26, w: lcw[3], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });
  s.addText('TIMEFRAME', { x: lcx[4], y: 1.26, w: lcw[4], h: 0.2, fontFace: 'Calibri', fontSize: 8, bold: true, color: MUTED, charSpacing: 0.5, margin: 0 });

  const RISK = { Low: GREEN, Medium: AMBER, High: RED };
  const TIME = { 'Quick win': GREEN, 'Medium term': AMBER, 'Long term': SLATE };
  const levers = [
    ['1', 'Automate high-labour operations', 'Labour above 30% of part cost — cobots or hard automation on repetitive tasks', '20%', 'Medium', 'Medium term'],
    ['2', 'Near-net-shape / better nesting', 'Material utilisation under 75% — pre-forms or CAD-optimised nesting toward 80–90%', '15%', 'Medium', 'Medium term'],
    ['3', 'OEE improvement programme (TPM)', 'OEE under 82% — planned maintenance and quicker changeovers toward the 85% mark', '12%', 'Low', 'Medium term'],
    ['4', 'Volume increase to dilute tooling', 'Tooling above 12% of part cost — doubling volume halves tooling per part', '10%', 'Low', 'Quick win'],
    ['5', 'Overhead negotiation, open-book', 'Overhead above 15% — benchmark against typical 10–15% tier-1 burden rates', '6%', 'Low', 'Quick win'],
    ['6', 'Competitive RFQ on margin', 'Margin above 12% — three-way RFQ with this should-cost as the floor price', '5%', 'Low', 'Quick win'],
    ['7', 'Multi-axis operation consolidation', 'More than 4 operations — a 5-axis centre folds several setups into one', '12%', 'Medium', 'Long term'],
    ['8', 'Lower-cost-region sourcing study', 'Process plus labour above 30% of cost — regional rates create real arbitrage', '18%', 'High', 'Long term'],
    ['9', 'DFM design review', 'Backstop — added whenever fewer than 5 levers have fired', '5%', 'Low', 'Quick win'],
    ['10', 'Annual volume re-commitment', 'Backstop — added whenever fewer than 6 levers have fired', '3%', 'Low', 'Quick win'],
  ];
  levers.forEach((r, i) => {
    const y = 1.55 + i * 0.425;
    s.addShape('roundRect', { x: 0.5, y, w: 12.33, h: 0.385, fill: { color: i >= 8 ? PAGE : CARD }, line: { color: i >= 8 ? MUTED : LINE, width: 1, dashType: i >= 8 ? 'dash' : 'solid' }, rectRadius: 0.06 });
    s.addText(r[0] + ' · ' + r[1], { x: lcx[0], y, w: lcw[0], h: 0.385, fontFace: 'Calibri', fontSize: 9.4, bold: true, color: NAVY, margin: 0, valign: 'middle' });
    s.addText(r[2], { x: lcx[1], y, w: lcw[1], h: 0.385, fontFace: 'Calibri', fontSize: 8.8, color: SLATE, margin: 0, valign: 'middle' });
    s.addText(r[3], { x: lcx[2], y, w: lcw[2], h: 0.385, fontFace: 'Calibri', fontSize: 10, bold: true, color: TEAL, margin: 0, valign: 'middle' });
    s.addText(r[4], { x: lcx[3], y, w: lcw[3], h: 0.385, fontFace: 'Calibri', fontSize: 9, bold: true, color: RISK[r[4]], margin: 0, valign: 'middle' });
    s.addText(r[5], { x: lcx[4], y, w: lcw[4], h: 0.385, fontFace: 'Calibri', fontSize: 9, bold: true, color: TIME[r[5]], margin: 0, valign: 'middle' });
  });

  s.addShape('roundRect', { x: 0.5, y: 6.02, w: 12.33, h: 0.82, fill: { color: NAVY }, rectRadius: 0.1 });
  s.addText([
    { text: 'Honest footnote:  ', options: { bold: true, color: '9FB6E0' } },
    { text: 'levers 9 and 10 are deliberate backstops so no part ever leaves with an empty ideas list — every estimate carries at least 5–6 actions, each sized in pounds when it reaches the report. Savings shown are each lever’s cap; the actual figure is computed from the part’s own numbers.', options: { color: 'FFFFFF' } },
  ], { x: 0.85, y: 6.1, w: 11.65, h: 0.66, fontFace: 'Calibri', fontSize: 11.5, margin: 0, valign: 'middle' });
  footer(s, ++PG);

  s.addNotes(
    'The last piece: how findings turn into an action list — what the tool calls cost-optimisation levers, and what I have been calling idea generation. ' +
    'Ten levers, and by now the triggers will look familiar because they read the same ten parameters as everything else. Labour above thirty percent suggests automation. Utilisation under seventy-five suggests near-net-shape or better nesting. OEE under eighty-two suggests a TPM programme. Tooling above twelve percent — raise the volume, which was exactly the bumper amortisation story. Overhead above fifteen and margin above twelve are the two commercial quick wins. More than four operations points at multi-axis consolidation — and remember from the routing slide, the optimiser now checks whether consolidation actually wins on the part before recommending it. Heavy conversion cost points at a regional sourcing study — the one lever the tool grades high-risk, because moving a part is never free. ' +
    'Each idea comes out with three tags: a saving, computed from the part’s own numbers and capped at the figure on the slide; a risk grade; and a timeframe — quick win, medium term or long term. The quick wins are the ones a buyer can raise in the next supplier meeting without any engineering change at all. ' +
    'And the honest footnote, because someone will ask: levers nine and ten are backstops. If fewer than five ideas have fired naturally, the tool adds a formal DFM review and a volume re-commitment conversation, so no part ever leaves with an empty list. They are labelled as what they are — sensible defaults, not discoveries. ' +
    'That is the whole appendix: ten parameters, thirty-six rules, ten geometry advisors, ten levers. Every threshold you have seen is a line of plain arithmetic in the tool’s source code, and it produces the same answer every time it runs.'
  );
}
