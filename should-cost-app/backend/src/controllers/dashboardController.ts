...
export async function getDashboard(_req: Request, res: Response): Promise<void> {
  try {
    ...
    // ── "New this month" activity counts ─────────────────────────────────────
    // These are honest inflow metrics (rows created since the start of this
    // calendar month), NOT deltas of the point-in-time totals shown on the
    // tiles — computing a true month-over-month delta of a "currently open"
    // count would need historical snapshots we don't store. Each query is
    // independent, so run them in parallel rather than adding serial latency
    // to this frequently-polled endpoint. A missing table resolves to 0.
    ...
    const [new_negotiations_this_month, new_quotes_this_month, new_high_variance_this_month] =
      await Promise.all([
        scalar(`SELECT COUNT(*) AS n FROM negotiation_target
                WHERE created_at >= date_trunc('month', CURRENT_DATE)`),
        scalar(`SELECT COUNT(*) AS n FROM supplier_quote_header
                WHERE COALESCE(submitted_at, created_at) >= date_trunc('month', CURRENT_DATE)`),
        scalar(`SELECT COUNT(*) AS n FROM comparison_snapshot
                WHERE ABS(COALESCE(variance_pct, 0)) > 15
    ...
    res.json({
      ...
      recent_comparisons,
      recent_quotes,
      new_negotiations_this_month,
      new_quotes_this_month,
      new_high_variance_this_month,
    });
  } catch (err) {
