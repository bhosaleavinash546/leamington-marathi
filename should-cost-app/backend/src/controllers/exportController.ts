...
// ─────────────────────────────────────────────────────────────────────────────
// Shared CSV sender — runs the query, maps rows, streams the file, and
// gracefully returns an empty (header-only) CSV when the underlying table or
// column is missing (fresh DB / un-migrated volume) instead of a 500.
// ─────────────────────────────────────────────────────────────────────────────
...
async function sendCsv(
  ...
  const filename = `${opts.filenameStem}-${new Date().toISOString().slice(0, 10)}.csv`;
  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename=${filename}`);
  try {
    const { rows } = await pool.query(opts.query);
    res.send(toCsv(opts.headers, rows.map(opts.mapRow)));
  } catch (err) {
    const pg = err as { code?: string };
    // 42P01 = undefined_table, 42703 = undefined_column
    if (pg.code === '42P01' || pg.code === '42703') {
      res.send(toCsv(opts.headers, []));
      return;
    }
    throw err;
  }
...
// ─────────────────────────────────────────────────────────────────────────────
export function exportNegotiationsCsv(_req: Request, res: Response): Promise<void> {
  return sendCsv(res, {
    query: `SELECT nt.id, p.part_number, p.description AS part_description,
...
// ─────────────────────────────────────────────────────────────────────────────
export function exportAcrCsv(_req: Request, res: Response): Promise<void> {
  return sendCsv(res, {
    query: `SELECT at.id, p.part_number, p.description AS part_description,
...
// ─────────────────────────────────────────────────────────────────────────────
export function exportCommodityPricesCsv(_req: Request, res: Response): Promise<void> {
  return sendCsv(res, {
    query: `SELECT id, material_name, material_code, price_per_unit, unit, currency,
...
// ─────────────────────────────────────────────────────────────────────────────
export function exportShouldCostListCsv(_req: Request, res: Response): Promise<void> {
  return sendCsv(res, {
    query: `SELECT sch.id, p.part_number, p.description AS part_description,
    ...
    filenameStem: 'should-costs',
  });
}

// ─────────────────────────────────────────────────────────────────────────────
// CSV — Quotes List  GET /api/export/quotes.csv
// (supplier_quote_header has no `notes` column — use rfq_number instead)
// ─────────────────────────────────────────────────────────────────────────────
export function exportQuotesCsv(_req: Request, res: Response): Promise<void> {
  return sendCsv(res, {
    query: `SELECT sqh.id, p.part_number, p.description AS part_description,
                   s.name AS supplier_name, sqh.status, sqh.currency,
                   sqh.total_price, sqh.rfq_number, sqh.submitted_at, sqh.created_at
    ...
    headers: [
      'ID','Part Number','Description','Supplier','Status','Currency',
      'Total Price','RFQ Number','Submitted At','Created At',
    ],
    mapRow: (r) => [
      r.id, r.part_number, r.part_description, r.supplier_name, r.status,
      r.currency, r.total_price, r.rfq_number, r.submitted_at, r.created_at,
    ],
    filenameStem: 'quotes',
  });
}
