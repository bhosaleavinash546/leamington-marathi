...
import { Search, ChevronDown, ChevronRight, HelpCircle, BookOpen, Mail, Zap, Terminal, Download, Globe, Lock, Shield, Cpu, BarChart3, Sparkles, Map, Share2, Package, Smartphone, Wand2, Layers, CheckCircle, Boxes } from 'lucide-react';
...
const FAQ: FaqSection[] = [
  ...
  {
    category: 'DFM / DFA Studio',
    icon: Boxes,
    items: [
      { q: 'What does the DFM / DFA Studio do?', a: 'Upload a 3D CAD file and it measures the part, then checks it against a cited catalogue of manufacturability rules and prices what it finds. Two halves: DFM (Design for Manufacture) on a single part — draft angles, undercuts, wall thickness, holes, pockets, ribs, bends, machining setups — and DFA (Design for Assembly) on a multi-part assembly, which decomposes the STEP file into solids and scores how hard it is to put together. Find it at Tools → DFM / DFA Studio (/dfm-studio).' },
      { q: 'What file formats does it need, and why not STL?', a: 'STEP (.step / .stp) or IGES (.igs / .iges). It will refuse an STL, and that refusal is the honest answer rather than a limitation: an STL is a triangle mesh with no topology, so it contains no faces, no edges, no hole diameters and no surface types. Draft, undercuts and feature recognition are all topology questions. A .sldprt, .catpart or .x_t needs a licensed kernel we do not ship — export STEP from your CAD tool, which every major package can do.' },
      { q: 'Where do the numbers come from — is the AI writing them?', a: 'No, and deliberately so. No LLM is on this path at all. Geometry is measured by an OpenCascade kernel: draft angles and wall thickness on the tessellation (which works on freeform surfaces, where a plane-and-cylinder analysis measures almost nothing on a real casting), holes and features from the B-rep topology. Every cost figure comes from the same deterministic engines the rest of BrainSpark uses. The manufacturability score is arithmetic over the rule findings, not an AI opinion.' },
      { q: 'What does "NOT EVALUATED" mean on a rule?', a: 'That the measurement that rule needs was not available on your geometry — so it was neither passed nor failed, and the reason is printed. This is the point of the whole feature. A rule reported as a pass because nobody could check it would be a clean bill of health you never earned. Each process family shows its coverage (say "5 of 7 rules evaluated") beside its score, and the score is blank rather than 100 when nothing could be checked at all.' },
      { q: 'Which processes are covered, and how are the thresholds justified?', a: 'Four families — CNC machining, injection moulding, high-pressure die casting, and sheet metal / stamping — with 26 rules in total, each carrying its threshold, unit, rationale, suggested fix and a cited source. You can read the whole catalogue before uploading anything — including the five drivers that deliberately have NO rule (tool reachability, tolerance stack-up, sink and warp, blank nesting, press tonnage), each stating the measurement it would need. A rule with nothing to measure could only ever report NOT EVALUATED, so it is named as a gap rather than shipped as a permanent blank. They are industry design guidelines, not laws of physics: a capable supplier will beat several of them, so treat a finding as the start of a conversation with your supplier rather than a verdict.' },
      { q: 'Why do some findings show a cost and others do not?', a: 'Because the engines genuinely model some drivers and not others. Wall thickness drives the cooling-limited cycle, setups drive machining cost, bend count drives forming, and an over-thick rib is material that can be measured and re-costed — those come back with a per-part and annual figure. Draft angle, undercuts, corner radii and warp risk are tooling, die-life or simulation outcomes the piece-price engines do not model, so those say "not priced" with the reason. That is not the same as zero — an undercut buys tooling, and it may well be the expensive one. Where the literature gives a cost range it is passed through explicitly labelled as a citation, never as an engine result.' },
      { q: 'How does the DFA half work, and why is the index sometimes blank?', a: 'It decomposes a multi-solid STEP assembly and measures each part: mass, size, contacts, repeated instances by shape signature, and the alpha/beta rotational symmetry that drives handling time — measured by actually rotating each solid and intersecting it with itself, not guessed from inertia. What geometry cannot answer is Boothroyd’s three minimum-part questions, which are about function and intent: does this part move relative to the ones it assembles to, must it be a different material, must it be separate for assembly or service? Geometry proposes an answer at low confidence and YOU confirm it with the Y/N toggles. Until every part is answered the theoretical minimum and the design-efficiency index stay blank rather than being quietly assumed.' },
      { q: 'Is this Boothroyd Dewhurst DFMA?', a: 'It follows the same public method, and it is not their software. Their handling and insertion time tables are copyrighted and licensed with their product, so we do not reproduce them. The time model here is ours: MTM-structured, with every coefficient exposed as data so your plant can calibrate it against real line times, and its provenance is printed in the report.' },
      { q: 'What if the analysis could not measure something?', a: 'It says so in an "Analysis limits" block at the top of the page and both exports, before any numbers. Units that look like metres rather than millimetres, a multi-body file merged into one solid, a tessellation that hit its triangle budget, an assembly too large to measure every symmetry. A metre-scale file is treated as blocking: every dimensional finding is withheld and re-listed as not-evaluated, because a "0.05 mm wall" computed at the wrong scale is worse than no finding at all.' },
      { q: 'Can I export the analysis?', a: 'Yes — a branded PDF and a multi-sheet Excel workbook, both from the button at the top of the results. The Excel carries separate sheets for findings, rules that could not be evaluated, rules that passed, recognised features, ribs and the DFA part table, so you can filter and paste it straight into a supplier discussion.' },
    ],
  },
  {
  ...
  },
  {
    ...
    icon: Share2,
    items: [
      { q: 'How do workspaces and roles work?', a: 'Every account gets a personal workspace automatically. Workspace owners and admins can invite colleagues by email from Settings → Team; the invite sits pending and activates the moment that email signs up. Roles are owner, admin, member, and viewer. This is the v1 substrate — org-scoped project sharing builds on it next.' },
        ...
        {!search && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
              {[
                { icon: Boxes,      color: 'text-teal-400',   bg: 'bg-teal-500/10',   border: 'border-teal-500/20',  title: 'Part 360', desc: 'One part interrogated from every angle: upload the 3D model, 2D drawing and supplier quote with breakdown, and every engine measures it — should-cost, DFM routes, spec relaxation, 9-region sweep, volume curve. The result is a Cost Entitlement Waterfall (quote → commercial → specification → process → footprint premiums, every step an engine calculation that chains exactly, skipped steps saying why) and line-by-line quote forensics against the engine\'s own buckets. Ideas are then generated through evidence lenses and must cite the measured [E#]/[W#] lines — uncited ideas are visibly flagged. Waterfall and forensics slides export in the negotiation pack. See /part-360.' },
                { icon: Boxes,      color: 'text-cyan-400',   bg: 'bg-cyan-500/10',   border: 'border-cyan-500/20',  title: 'DFM / DFA Studio', desc: 'Upload a STEP or IGES file and get a measured manufacturability report — no AI anywhere on the path that produces the numbers. An OpenCascade kernel measures draft angles and wall thickness on the tessellation (so it works on freeform castings, not just planes and cylinders), and recognises holes, counterbores, countersinks, pockets, slots, chamfers, fillets, ribs and sheet-metal bends from the topology. 26 cited rules across machining, injection moulding, HPDC and stamping, each reporting pass, fail or NOT EVALUATED with the reason — findings are priced by the same engines the rest of the tool uses, or say plainly that they cannot be. Assembly DFA decomposes a multi-solid STEP, measures each part\'s alpha/beta symmetry by rotating it, groups repeated parts by shape signature, and withholds the design-efficiency index until you answer the three minimum-part questions. Branded PDF and multi-sheet Excel. See /dfm-studio.' },
                { icon: Sparkles,   color: 'text-violet-400', bg: 'bg-violet-500/10', border: 'border-violet-500/20', title: 'Deep Mode + 3 new methods', desc: 'Opt-in Deep Mode on Analyze: a 3-persona expert panel critiques every idea, a pairwise Elo tournament ranks them, and engine-contradicted ideas get one verified repair pass. Innovation Studio gains FAST Function-Cost Matrix, Spec & Tolerance Challenge (CTQs code-locked, savings engine-computed) and Teardown Delta. Should-Cost can now turn a quote gap into bucket-targeted gap-closure ideas. Marketplace adds duplicate warnings, theme clusters, and a coverage map; the pipeline gets stage-gate scorecards; Patent Watch cites real PatentsView records when a key is configured.' },
                ...
                { icon: Zap,        color: 'text-cyan-400',   bg: 'bg-cyan-500/10',   border: 'border-cyan-500/20',  title: 'Wiring-Harness Costing', desc: 'A dedicated parametric harness model: copper mass from gauge mix × circuit lengths, connector/terminal/splice economics, labour-minutes, and an honesty band. Open it from Wiring Harness under Cost & analyze.' },
