...
export const ICE_COMPONENTS: IceComponent[] = [
  {
    ...
    levers: [
      ...
      { action: 'Integrated exhaust manifold in cylinder head (IEM): delete standalone manifold', saving: '£34-68/unit, faster warm-up (5-8% fuel saving)', conf: 'verified', bench: 'Ford EcoBoost, VW EA211 evo, BMW B48' },
      ...
      { action: 'Hollow camshaft (assembled) vs solid forged — same stiffness, 25-30% cam mass', saving: '£6.8-13/cam in material, enables oil-through spray bar', conf: 'verified', bench: 'BMW, Audi TFSI — standard on DOHC >2.0L' },
      { action: 'VVT phaser integration into single actuator hub — delete separate OCV valve', saving: '£10-17/unit parts + £3.4-5.1 assembly time', conf: 'estimated', bench: 'Denso unified VVT, BorgWarner eTVT system' },
    ],
  },
  {
    id: 'exhaust-system',
    ...
    levers: [
      { action: 'Reduce PGM loading via advanced washcoat (TWC) — Pd/Rh ratio optimisation', saving: '15-30% catalyst metal cost (Pd ~£26/g, Rh ~£170/g)', conf: 'benchmarked', bench: 'BASF, Umicore Gen5 washcoat — 30% Pd reduction' },
      ...
      { action: 'Common centre-pipe section across 2 engine variants — shared tooling', saving: '£153-298K tooling amortised across higher volumes', conf: 'estimated', bench: 'VW Group MQB — shared NF pipe + single muffler platform' },
      { action: 'Acoustic resonator delete via active exhaust control valve (motor only models)', saving: '£30-47/vehicle NRE + content on non-performance spec', conf: 'estimated', bench: 'Ford Mustang active exhaust delete option (base trim)' },
    ],
  },
  {
    id: 'turbo-system',
    ...
    levers: [
      { action: 'Twin-scroll single turbo vs twin turbo layout — same output, fewer parts', saving: '£153-238/unit vs twin turbo configuration', conf: 'verified', bench: 'BMW B58 vs N54, Ford EcoBoost 2.7L twin-scroll' },
      { action: 'Integrated wastegate in turbine housing — delete external actuator', saving: '£21-38/unit + packaging benefit', conf: 'verified', bench: 'Garrett GTX, BorgWarner Regulated 2-Stage (R2S)' },
      { action: 'Water-cooled bearing housing — enables oil-free idle down, delete turbo timer', saving: '£13-21 content reduction + warranty improvement', conf: 'benchmarked', bench: 'All modern OEM turbos — standard feature from 2019+' },
      { action: 'Air-to-air FMAC intercooler vs water-cooled WCAC on base/mid trim', saving: '£68-111/unit cost saving on lower-output applications', conf: 'estimated', bench: 'VW EA888 Gen3 uses WCAC — base trims can use FMAC' },
  ...
  },
  {
    ...
    levers: [
      { action: 'Combined port + GDI (CPDI) injection delete port injectors on base spec', saving: '£30-47/vehicle by removing port injection on entry trims', conf: 'verified', bench: 'VW EA888 Gen3B vs Gen3: delete port injectors on base MY' },
      ...
      { action: 'Modular 6-layer HDPE fuel tank (standard geometry) across platform — shared blow-mould tool', saving: '£213-425K tooling saving across 2 derivatives', conf: 'estimated', bench: 'Toyota TNGA shared tank geometry (GR86/BRZ vs SUV derivs)' },
      { action: 'Delete high-pressure fuel pump accumulator on systems with electronic pressure control', saving: '£6.8-12/unit part + 1 part count reduction', conf: 'estimated', bench: 'Bosch HDEV5 — integrated pressure control deletes accumulator' },
  ...
  },
  {
    ...
    levers: [
      { action: 'Electric coolant pump (EWP) + map thermostat — delete belt-drive parasitic drag', saving: '£17-30 net save after EWP premium, 2-3% fuel saving at warm-up', conf: 'verified', bench: 'BMW N20/B48, Mercedes M264 — electric pump standard from 2016' },
      ...
      { action: 'Low-temperature (LT) coolant loop share with charge air cooling — delete dedicated WCAC loop', saving: '£38-64/vehicle by combining LT circuits', conf: 'estimated', bench: 'Ford EcoBoost Gen3 shared LT coolant loop' },
    ],
  },
];
...
export const ICE_TRENDS: IceTrend[] = [
  { id: 'eu7-compliance', title: 'EU7 Aftertreatment Requirements', description: 'EU7 mandates stricter PN/NOx limits (0.1°C cold-start window), requiring close-coupled TWC + GPF with electrically heated catalyst (eCAT) on all petrol engines from 2026 light duty.', status: 'Mainstream', impact: 'eCAT adds £30-51/vehicle — offset partially via thinner-wall GPF and Pd/Rh optimisation.' },
  { id: 'mhev-integration', title: '48V MHEV Integration on ICE', description: 'Belt-starter generator (BSG/RSG) becoming standard on C-segment+ ICE: 12-15% fuel saving vs conventional; eCAT function transferred to BSG catalyst heating.', status: 'Mainstream', impact: 'MHEV premium ~£298-510 partially offset by downsized engine, delete idle stop starter.' },
  ...
  { id: 'pgm-reduction', title: 'Platinum Group Metal Reduction', description: 'Advanced washcoat formulations (Pd-rich, Rh-reduction) reducing PGM loading 20-35% while maintaining EU6d temp light-off performance. Critical given Rh volatility (£128-213/g range).', status: 'Mainstream', impact: '20-35% catalyst system cost reduction at material level.' },
  { id: 'igcc-combustion', title: 'Ignition Gasoline Compression (IGCI)', description: 'Pre-chamber ignition enabling very lean combustion (λ=2), reducing fuel consumption 12-18% vs stoichiometric. BMW Neue Klasse ICE range-extender applies this.', status: 'Emerging', impact: 'NRE investment ~£43-68M for combustion system and injector development.' },
  { id: 'integrated-exhaust', title: 'Cylinder Head with Integrated Exhaust Manifold', description: 'IEM (Integrated Exhaust Manifold) in the head eliminates a separate manifold part, reduces gasket interface, enables water-cooling of exhaust port reducing need for oil cooling.', status: 'Mainstream', impact: '£34-68/vehicle parts saving + faster catalyst light-off (2-3% CO2).' },
  { id: 'variable-displacement', title: 'Cylinder Deactivation (CDA/VDE)', description: 'Cylinder deactivation on 4/6-cylinder via hydraulic valve train deactivation: 5-8% fuel saving in highway cruise. Becoming standard on all 4-cyl ICE with MHEV.', status: 'Emerging', impact: 'Honda i-VTEC, GM AFM — NRE ~£21-34M per engine family.' },
...
export const ICE_OEM_BENCHMARKS = [
  { oem: 'Toyota', model: 'GR Corolla 1.6T / THS-II 2.5 Hybrid', moves: ['GR Corolla G16E-GTS 1.6T 3-cylinder: 224 kW at 6,500 rpm — demonstrating 3-cyl viability for performance, delete 1 cylinder + balance shaft vs 4-cyl baseline', 'THS-II (Toyota Hybrid System): 2.5 Atkinson cycle at 41% thermal efficiency — highest mass-market ICE thermal efficiency 2024, Pd-only TWC eliminates Rh', 'Integrated intake + exhaust camshaft (gear-driven): delete timing chain + separate tensioner assembly, −4 parts per engine', 'EU7 road map: solid electrolyte catalyst (ceramic substrate eCAT) for sub-0°C light-off — 35 g Pt replacement with Pd-rich washcoat saving £15/vehicle'] },
  ...
  { oem: 'Mercedes-Benz', model: 'M254 4-cyl / M256 6-cyl MHEV / OM654 Diesel', moves: ['M254 MHEV: 48V integrated starter-generator (ISG) on gearbox bell-housing — 16 kW recuperation, delete 12V alternator entirely from engine ancillaries', 'IEM (Integrated Exhaust Manifold in head): M254 EU7 — exhaust port water-cooling via cylinder head jacket, faster TWC light-off, 3% CO2 saving', 'PGM optimisation: Pd:Rh ratio shift 4:1 → 6:1 on M254 TWC (BASF Catalysts) — saves 0.3 g Rh/engine @ £187/g = £56/engine at Rh peak pricing', 'OM654 diesel 4-cyl: delete 2 balance shafts at Gen2 (NVH target achieved by revised crankshaft balance factor) — 1.4 kg saving, 2 parts deleted'] },
  ...
  { oem: 'Hyundai / Kia', model: 'Smartstream G2.5 / Lambda III V6 / CVVD Engine', moves: ['CVVD (Continuously Variable Valve Duration) on Smartstream G1.6T: first mass-production CVVD — 4% efficiency gain over CVVT, enables Atkinson cycle at cruise without cam profile switch', 'Smartstream G2.5 GDI: Pd-dominant TWC (4.8 g Pd, 0.4 g Rh vs 3.6/0.8 on outgoing Lambda II) — 50% Rh reduction, saving £30/TWC at peak Rh pricing', 'Lambda III V6 3.5T: delete balance shaft (inline-60° V6 inherently balanced) — 0.6 kg + 2 parts saving vs previous V6 Lambda requiring balance shaft', 'Integrated MHEV (48V RSG, Hyundai-Kia in-house motor): replace Bosch MHEV with in-house unit — £32/system saving at 300K units/yr volume'] },
];
