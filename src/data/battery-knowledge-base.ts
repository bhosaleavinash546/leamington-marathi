...
export const BATTERY_COMPONENTS: BatteryComponent[] = [
  ...
  },
  {
    ...
    levers: [
      ...
      { t: 'Delete inter-module HV connector (direct busbar routing)', save: '€3-5/connector × N modules', bench: 'Tesla Model 3, BYD Blade', bodyStyles: ['universal'], conf: 'benchmarked', note: 'CTP/CTC direct cell-to-busbar eliminates inter-module connectors — requires pack-level serviceability planning' },
    ],
  },
  {
    id: 'bms',
    ...
    levers: [
      ...
      { t: 'Wireless BMS (wBMS): delete cell monitoring harness', save: '€8-15 per kWh (harness)', bench: 'GM Ultium (Analog Devices wBMS)', bodyStyles: ['sedan','suv','pickup','universal'], conf: 'verified', note: 'Proven in production: GM Hummer EV/Lyriq. Eliminates harness looms between modules. Pack reassembly simplified' },
      { t: 'Integrate BMS function into VCU/Domain controller', save: '€40-80 standalone ECU cost', bench: 'Rivian R1T/R1S, Lucid Air', bodyStyles: ['sedan','suv','pickup'], conf: 'estimated', note: 'High-performance compute node absorbs BMS: one fewer ECU, shared HW platform, one validation effort' },
  ...
  },
  {
    ...
    levers: [
      ...
      { t: 'Delete chiller for PHEV (phase-change TIM substitution)', save: '€80-150 chiller + plumbing', bench: 'Stellantis PHEV platforms', bodyStyles: ['hatchback','sedan','suv','crossover'], conf: 'estimated', note: 'PHEV partial-charge cycles: phase-change TIM buffers heat spikes, simplifies cooling loop vs full BEV chiller' },
  ...
  },
  {
    ...
    levers: [
      ...
      { t: 'CTP: reduce HV connectors (direct cell-to-busbar)', save: '€3-5 × N inter-module connectors', bench: 'Tesla Model 3, BYD Blade CTP', bodyStyles: ['universal'], conf: 'verified', note: 'CTP eliminates inter-module connections — busbar routes directly over cells. Fewer HVIL loops required' },
...
export const BATTERY_TRENDS: BatteryTrend[] = [
  ...
  { t: 'Wireless BMS (wBMS): delete monitoring harness', status: 'Emerging', save: '€8-15/kWh harness', dir: 'Analog Devices + GM Ultium: first high-volume wBMS in production. Eliminates cell voltage monitoring wiring loom. Simplifies pack assembly. Remaining challenge: EMI validation in dense cell environment.' },
  ...
  { t: 'Sodium-ion cells for city car / entry BEV', status: 'Next-Gen', save: '€35-45/kWh target', dir: 'CATL Naci Na-ion in production (2024) for Chery BEV. BYD Na-ion prototypes. Lower cost than LFP. 0°C performance advantage. 100-130 Wh/kg — city car range adequate. Zero lithium, cobalt, nickel.' },
