...
export const EXTERIOR_COMPONENTS: ExteriorComponent[] = [
  {
    ...
    levers: [
      ...
      { action: 'Common rear/front bumper beam section geometry (symmetric crash can mount) — shared tool', saving: '£170-340K tooling saving on F/R beam family sharing mount geometry', conf: 'estimated', bench: 'Toyota TNGA — shared bumper beam mount brackets F/R' },
      { action: 'PP-EPDM fascia replace with PP-GF15 for structural integration of sensors/PDC brackets', saving: 'Delete 4-6 clip-on sensor brackets; £6.8-12 assembly saving', conf: 'benchmarked', bench: 'Valeo bumper module — integrated PDC mounting in fascia' },
      { action: 'Delete lower NVH deflector on base trim (retain aero shield only on premium with active aero)', saving: '£10-19/vehicle NVH under-bumper deflector delete on price-sensitive spec', conf: 'estimated', bench: 'VW Golf base spec — delete lower air deflector vs GTI with active aero flap' },
    ],
  },
  {
    id: 'lighting',
    ...
    levers: [
      { action: 'Zoned LED headlight replacing full Matrix ADB on base trim — meets legal obligation at lower cost', saving: '£153-272/unit zoned LED vs ADB Matrix (7-12 segment vs 84 pixel)', conf: 'verified', bench: 'Osram LED chip cost: ADB matrix 84-LED vs 4-zone LED — 3× cost difference' },
      { action: 'Single LiDAR-free LED strip DRL (delete individual LED positions, single injection-moulded wave guide)', saving: '£13-24/headlight unit in LED component and fixture assembly saving', conf: 'benchmarked', bench: 'Valeo LED guide DRL — single injection-moulded PMMA strip vs discrete LEDs' },
      { action: 'Front and rear lamp shared inner optic carrier (shared bezel moulding geometry)', saving: '£128-238K shared tooling on inner lamp housing', conf: 'estimated', bench: 'BMW Group strategy: shared optic carrier across G20/G26 derivatives' },
      { action: 'Delete front fog lamp (replace with cornering function in main headlight — legislative change 2024)', saving: '£24-38/vehicle fog lamp delete per EU UNECE ECE R48 amendment (2024)', conf: 'verified', bench: 'EU 2024 regulation: cornering function in ADB headlight replaces standalone fog lamp requirement' },
      { action: 'Tail lamp common outer lens across hatchback/estate using oversize lens + clip-in infill', saving: '£102-187K tooling save on derivative programmes with shared outer lens', conf: 'estimated', bench: 'VW Golf variant/estate — lens overhang cover approach for boot extension' },
    ],
  },
  {
    id: 'glass-glazing',
    name: 'Glass & Glazing',
    levers: [
      { action: 'Acoustic interlayer windscreen (PVB acoustic) — delete separate NVH damping pad on A-pillar', saving: '£3.4-6.8/vehicle A-pillar NVH pad delete; improved high-freq attenuation', conf: 'verified', bench: 'AGC Planibel Acoustic — standard BEV windscreen with acoustic PVB layer' },
      { action: 'Thermal comfort glazing (heat-reflecting) side glass — reduces HVAC load 8-12% in summer', saving: 'HVAC compressor right-size: £21-34 compressor displacement reduction', conf: 'benchmarked', bench: 'Saint-Gobain EasyCool — deployed on Renault Espace, Peugeot 3008' },
      { action: 'Electrochromic panoramic glass (switchable tint) replacing mechanical blind — delete mechanism', saving: '£72-119 mechanism saving offset by £55-94 electrochromic premium = £17-26 net saving', conf: 'estimated', bench: 'Continental / View Inc electrochromic glass — BMW M3 panoramic, Mercedes S-Class' },
      { action: 'Heated windscreen (ITO coating) for faster defrost — reduce HVAC and idle time', saving: '3-5% BEV range improvement in winter via faster demist (less HVAC energy)', conf: 'benchmarked', bench: 'AGC Thermo-Coat — deployed on Ford Ranger, VW ID.7 — £30-47/windscreen premium' },
    ],
  },
  {
    id: 'wipers-washers',
    ...
    levers: [
      { action: 'Aero flat blade replacing conventional frame blade — aerodynamic noise reduction, longer life', saving: '£1.7-4.3/set material saving + warranty improvement (blade lift at >130km/h)', conf: 'verified', bench: 'Bosch AeroTwin — standard D-segment from 2019. Flat blade now industry standard' },
      { action: 'Single-arm wiper (RHD design delete: right-side post vs dual post) on SUV/hatchback', saving: '£13-21/unit (1 arm + motor vs 2) + windscreen sweep area improvement', conf: 'estimated', bench: 'Mercedes CLA W118 — single-arm wiper. Cost case at >150K/yr justified' },
      { action: 'Rain sensor integrated into camera module (forward camera uses wiping algorithm)', saving: '£15-26/vehicle standalone rain sensor delete — function absorbed by ADAS camera', conf: 'theoretical', bench: 'Tesla Vision-based wiper control — no separate rain sensor (2021+ Autopilot camera)' },
      { action: 'Heated washer nozzle delete on markets with >0°C average winter (replace with fluid additive spec)', saving: '£6.8-13/vehicle heated nozzle delete on APAC/MENA market specifications', conf: 'estimated', bench: 'Toyota Land Cruiser regional specification — climate-based content deletion' },
    ],
  },
];

export const EXTERIOR_TRENDS: ExteriorTrend[] = [
  { id: 'aero-optimisation', title: 'Aerodynamic Drag Reduction for BEV Range', description: 'Cd reduction is the primary BEV range lever. Active aero grille shutters, underbody panels, flush door handles, aero wheels targeting Cd ≤0.20. Each 0.01 Cd saving adds ~3-5km range.', status: 'Mainstream', impact: 'Cd 0.22 → 0.19: +15km WLTP range. £30-55/vehicle aero content investment.' },
  { id: 'matrix-led', title: 'Matrix/Pixel LED Headlights (Glare-Free High Beam)', description: 'ADB Matrix LED (84-128 pixels) providing glare-free high beam — Euro NCAP 2026 ADAS assessment. ZKW/Marelli/Hella all offering pixel modules. Cost declining 8-12%/yr.', status: 'Mainstream', impact: 'ADB Matrix cost: £187-323/unit. NCAP 2026 advanced lighting gives +1.5 star in ADAS score.' },
  { id: 'camera-mirrors', title: 'Camera-Based Wing Mirrors (CSMS)', description: 'Camera Surround Monitor System replacing physical wing mirrors: EU type-approval from 2024. Cd improvement 0.005-0.010. Lexus ES, Honda e, Audi e-tron deployed. £238-340 premium.', status: 'Emerging', impact: 'Cd improvement worth 8-15km BEV range. Mirror-to-camera cost: net £153-255 premium on BEV.' },
  { id: 'flush-handles', title: 'Flush Retractable Door Handles', description: 'Electric flush door handles (retracted position) improve Cd by 0.002-0.005 and are a design signature. Standard on Tesla, Polestar, Lucid, Rimac. Power mechanism adds complexity.', status: 'Mainstream', impact: 'Door handle cost: conventional £15-24 vs flush £38-64. Net Cd/range gain partially offsets premium.' },
  { id: 'led-signature', title: 'Full-LED Signature Lighting as Brand Differentiator', description: 'Brand-signature LED DRL evolving into full-light language strategy: day, night, welcome animations. Software-defined lighting (OTA updates) replacing physical programming. Standard from C-segment.', status: 'Mainstream', impact: 'LED DRL system: £30-55/unit. Software animation: £0 marginal. Brand equity value high.' },
  { id: 'eu7-gpf', title: 'EU7 Brake Dust Filters (BDPF)', description: 'EU7 mandates brake dust particle filter from 2027 (4 mg/km PN limit). £13-21/wheel added content. ITELMA/Mann+Hummel suppliers qualifying. Electric park brake integration requirement.', status: 'Emerging', impact: 'Mandatory 2027 EU LD vehicles. Cost: £51-85/vehicle (4 wheels). Offset by disc right-sizing on BEV.' },
];
...
export const EXTERIOR_MFG_LEVERS: ExteriorMfgLever[] = [
  ...
  { id: 'washer-nozzle-common', name: 'Common Washer Nozzle Injection Tool Across 3 Models', description: 'Single injection tool (8-cavity) for PP washer nozzle body serves 3 vehicle models via interchangeable jet inserts (45°/60°/90° spray angle). Tool: £72.3K vs 3 model-specific tools at £179K. Volume pooling (3 models, 240K units/yr): PP compound cost −5%.', saving: '£106K tooling NRC; 5% material cost saving from volume', status: 'Mainstream' as const },
...
export const EXTERIOR_OEM_BENCHMARKS = [
  { oem: 'Tesla', model: 'Model 3 / Model Y / Cybertruck', moves: ['Flush electric door handles (Model 3/Y): delete conventional handle mechanism — present position sensor triggers motor at 0.5 N touch, saves £6.8/vehicle vs conventional handle mechanism weight', 'Camera mirror (Cybertruck, EU approval 2024): CSMS camera replaces wing mirrors — Cd improvement 0.006, equivalent to +12 km WLTP range on RWD', 'One-piece front/rear bumper fascia (Model 3 Highland): single large PP-EPDM injection, delete 6 clip-on sensor bracket inserts — integrated parking sensor boss moulded into fascia', 'Active underbody aero panels (Model Y Long Range): front undertray + mid undertray + rear diffuser sealed to body — Cd 0.208 vs 0.220 without panels'] },
  { oem: 'Porsche', model: 'Cayenne E3 Facelift / Taycan / 911 GT3 RS', moves: ['4-LED DRL signature (Cayenne E3): 4 LEDs + 1 PCB replaces 12 LEDs + 3 PCBs — £15/headlamp saving + 2.4 min assembly saving per vehicle', 'OLED rear cluster (Taycan/Panamera): 6-tile OLEDWorks flat panel, 22 mm housing depth vs 95 mm LED cluster — 0.8 kg/pair lighter, tooling saving £357K', 'Active aero wing (GT3 RS): 4-position carbon DRS wing replaces fixed element — aerodynamic downforce control via ECU from ADAS lateral g-sensor', 'CFRP bonnet + bootlid (911 GT3): dry carbon compression-moulded — 4.2 kg bonnet saving, enables 52:48 weight distribution target'] },
