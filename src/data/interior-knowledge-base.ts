...
export const INTERIOR_MFG_LEVERS: InteriorMfgLever[] = [
  { id: 'eva-carpet-mould', name: 'One-Shot EVA Floor Carpet Moulding', description: 'EVA (ethylene vinyl acetate) composite carpet — acoustic layer + carrier + pile — co-moulded in a single 90-second press cycle at 180°C. Replaces 3-layer hand-laid assembly (barrier mat + PU decoupler + carpet). Labour saving: 8 min/vehicle. STC +3 dB vs layered system.', saving: '8 min assembly; €77/vehicle material vs 3-layer system', status: 'Mainstream' as const },
  ...
  { id: 'cockpit-beam-common', name: 'Common IP Cockpit Beam Injection Tool Across Platform', description: 'Standardise long glass-fibre PP cockpit beam geometry (cross-car beam) across sedan/SUV derivatives with geometry inserts for model-specific attachment bosses. Single tool €420k vs 3 model-specific tools €1.1M. Volume pooling: PA66-LGF30 compound at 280K units/yr reduces material cost 6%.', saving: '€680K tooling saving; 6% material cost from volume', status: 'Mainstream' as const },
  ...
  { id: 'jit-seat-seq', name: 'JIT Seat Sequencing Direct-to-Line from Seat Supplier', description: 'Seat supplier (Lear/Faurecia/Adient) delivers sequenced seats via VMI kanban directly to body marriage station — eliminates OEM internal seat storage buffer (500 m² warehouse space, 3 h buffer). Delivery window: ±8 min per vehicle. EDI trigger on body VIN scan.', saving: 'Delete 500 m² storage, 3 h buffer; €12-18 logistics cost', status: 'Mainstream' as const },
  ...
  { id: 'hvac-duct-common', name: 'Common HVAC Duct Architecture Across Platform (Shroud-Only Change)', description: 'Platform-common HVAC duct core geometry with model-specific shroud insert (3D-printed prototype, injection production) for outlet positioning changes. Single duct tool €180K vs 3 model-specific €450K. Core thermal performance (air volume/noise) unchanged across derivatives.', saving: '€270K tooling NRC saving; single homologation cycle', status: 'Mainstream' as const },
  ...
  { id: 'pp-nf-compression', name: 'PP-NF Door Carrier Blanket Compression Moulding (Replace Injection)', description: 'Natural-fibre (30% hemp/flax) PP door trim carrier moulded via compression moulding (180°C, 80 bar, 45 s) replacing injection moulding. Density 0.94 vs 1.04 g/cm³ (injection) — 10% lighter panel. Tooling: matched-die compression tool €95K vs injection tool €240K for same geometry.', saving: '10% carrier mass; €145K tooling NRC; 30 s cycle vs 60 s injection', status: 'Emerging' as const },
  { id: 'overhead-console-common', name: 'Common Overhead Console Architecture Across SUV/Sedan', description: 'Platform-shared overhead console carrier (map lights + SOS/assist button + sunroof switch substrate) with model-specific trim ring insert. Carrier tool: €180K × 1 vs €180K × 3 (sedan/SUV/estate). Switch substrate: same part number all variants. Map light position adapted via trim ring.', saving: '€360K tooling NRC; single MLA harness connector', status: 'Mainstream' as const },
  ...
  { id: 'ip-ai-vision', name: 'Inline AI Camera Colour/Grain Inspection on IP Surface', description: 'Chromasens AllPIXA evo line-scan camera (12k pixels, 12-bit colour depth) with Matrox Imaging AI model classifies IP surface colour deviation (ΔE >0.8 CIELAB), grain depth (gloss meter equiv.), and scratch (0.05 mm width) at 100% inline. Previous: 5% sample + visual jury.', saving: '5% sample → 100% inline; €45 rework saving on caught IP defects', status: 'Emerging' as const },
];
...
export const INTERIOR_OEM_BENCHMARKS = [
  ...
  { oem: 'BYD', model: 'Han EV / Sea Lion / Seal / Atto 3', moves: ['Rotating 15.6-inch display (Han EV): single motorised pivot (delete slide rail via offset geometry) saves €28/vehicle — display flips portrait/landscape in 3 s', 'Flat floor storage (Ocean platform CTB): tunnel deleted → 400 mm wide centre floor box with sliding lid replaces tunnel console', 'Integrated solar roof + interior blind: PDLC electrochromic glass replaces fabric sunblind + motor + cable (€79/vehicle saving) across Han/Seal', 'Yoke steering (Han EV 4th gen): flat-bottom wheel deletes lower spoke leather + switchgear — capacitive touch surface for all signals'] },
];
