...
export const CHASSIS_COMPONENTS: ChassisComponent[] = [
  {
    ...
    levers: [
      ...
      { t: 'McPherson → double-wishbone base variant delete upper arm', save: '£30-51/corner', bench: 'Ford Focus ST, Peugeot 508', bodyStyles: ['hatchback','sedan','universal'], conf: 'benchmarked', note: 'On non-performance/non-SUV: delete upper A-arm, use standard McPherson — cost advantage outweighs dynamic' },
  ...
  },
  {
    ...
    levers: [
      { t: 'Multi-link → CTBA (twist beam) for B/C-segment', save: '£128-213/axle', bench: 'VW Polo Mk6, Renault Clio V', bodyStyles: ['hatchback','crossover','mpv'], conf: 'verified', note: 'CTBA adequate for FWD sub-C. Saves lateral links + knuckle + subframe vs multi-link. NVH trade-off managed with bushing tuning' },
      ...
      { t: 'Delete adaptive (CDC) damping on base/mid trim', save: '£170-340/vehicle', bench: 'BMW 5 Series base, Audi A4 base', bodyStyles: ['sedan','suv','coupe','hatchback','crossover'], conf: 'verified', note: 'CDC only on Sport/M-Sport/S-Line trims — deletes actuator + ECU. Passive damper well-tuned suffices' },
      { t: 'Air spring → coil spring delete (non-load-levelling)', save: '£298-510/vehicle', bench: 'Land Rover Freelander vs Discovery', bodyStyles: ['suv','pickup','mpv'], conf: 'verified', note: 'Remove air spring on trim grades not requiring height adjust. Significant cost save; tuned coil spring ride parity possible' },
      { t: 'Delete rear-wheel steering (non-performance models)', save: '£153-238/vehicle', bench: 'BMW 7 Series base vs M760Li', bodyStyles: ['sedan','suv','coupe'], conf: 'benchmarked', note: 'RWS only justified on L-WB or performance — delete on standard WB variants. Electric actuator cost reduces as volume grows' },
    ],
  },
  {
    id: 'steering',
    ...
    levers: [
      ...
      { t: 'Delete 4-way electric column adjustment on base trim', save: '£38-68/vehicle', bench: 'Audi A3 base vs Sportback', bodyStyles: ['hatchback','sedan','suv','universal'], conf: 'verified', note: 'Manual reach/rake or 2-way electric sufficient for base — delete 2-axis motor + memory function' },
      { t: 'Integrate EPS ECU into vehicle domain controller', save: '£21-43 ECU cost', bench: 'Rivian R1T/R1S, BMW Neue Klasse', bodyStyles: ['universal'], conf: 'estimated', note: 'Centralised domain computing absorbs EPS function — delete standalone EPS ECU unit' },
  ...
  },
  {
    ...
    levers: [
      ...
      { t: 'Steel ARB drop links vs Al forged (non-performance)', save: '£10-15/link', bench: 'VW/Skoda base specification', bodyStyles: ['hatchback','sedan','mpv','crossover','universal'], conf: 'verified', note: 'Stamped + ball-jointed steel links: identical function at significantly lower cost for non-performance' },
...
export const CHASSIS_TRENDS: ChassisTrend[] = [
  { t: 'Brake-by-wire (BBW) for ADAS regen blending', status: 'Emerging', save: 'Enabler for L3+ ADAS', dir: 'Continental MK C2, ZF IBC: fully decoupled BBW for seamless regen integration. Mandatory for ADAS L3+. BMW Neue Klasse, GM Ultium from 2025. +£128-213/vehicle vs conventional.' },
  ...
  { t: 'Steer-by-wire (SbW) — mechanical column deletion', status: 'Emerging', save: 'Packaging + ADAS enabler', dir: 'BMW Neue Klasse 2025 first high-volume SbW. Lexus RZ450e (optional). No mechanical column → design freedom + ADAS integration. Adds redundancy cost £170-298 but saves column £68-102.' },
  { t: 'EURO 7 brake dust particle limits (2027)', status: 'Emerging', save: 'Design constraint', dir: 'Brake dust particle filter (BDPF) mandatory EU from 2027: £13-21/wheel. Drives disc size right-sizing (BEV regen reduces friction load). May render separate dust shield obsolete.' },
  { t: 'Rear-wheel steering (RWS) volume increase', status: 'Emerging', save: 'Delete on sub-premium', dir: 'ZF AKC, Porsche, BMW now offer RWS on C-segment+. But cost still £153-255. Delete on non-performance variants — justified only for L-WB wheelbase or sport grade.' },
  { t: 'Air suspension democratisation (B/C-segment)', status: 'Emerging', save: 'Variable', dir: 'Arnott, Continental making air suspension accessible at B-segment. Reduces cost from £510→£298. Load-levelling function key differentiator vs coil. Growing in MPV/SUV.' },
...
export const CHASSIS_MFG_LEVERS: ChassisMfgLever[] = [
  ...
  { id: 'hydroform-subframe', name: 'Hydroformed Rear Subframe Tubular Sections', description: 'Internal high-pressure hydroforming (280 bar) of rear subframe longitudinal tubes: closed cross-section 120×80 mm from 2.0 mm S420MC steel. Eliminates separate inner tube and longitudinal weld seam. Dimensional repeatability ±0.3 mm along 800 mm length. Line investment: £1.78M per press.', saving: '15-20% subframe mass; delete weld seam operation', status: 'Mainstream' as const },
  ...
  { id: 'disc-laser-mark', name: 'Laser Brake Disc Traceability Marking (Delete Label)', description: '1064 nm fiber laser marking on inboard disc face (batch, date, supplier, cavity number) at 0.3 mm character height. Replaces adhesive paper label (£0.1/disc, 8 s application per disc). Permanent traceability enables warranty root-cause in 2 h vs 2 weeks.', saving: '2-4% trace cost; permanent traceability in field', status: 'Mainstream' as const },
...
export const CHASSIS_OEM_BENCHMARKS = [
  ...
  { oem: 'Tesla', model: 'Model 3 Highland / Model Y', moves: ['Rear gigacasting (6,000T): 171 parts → 1 casting, cycle time 90 s, saves £298/vehicle BOM', 'Front structural casting integrates strut tower + subframe cradle — delete 5 welded assemblies', 'Flat floor enables right-sized brake discs (regen covers 0.3g): front disc 320 mm vs 360 mm', 'Camera-vision brake dust monitor (2025 EU7 road map) — no BDPF hardware, software-only approach'] },
