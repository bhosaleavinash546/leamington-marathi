...
// ─── News Section — extracted to src/ui/panels/news.ts ───────────────────────
// showNews() is imported at the top of this file from './panels/news.js'
...
const CAD_AI_DEMOS: Record<string, CADAnalysisResult> = {
...
function saveScenarioFromModal(): void {
  ...
  // Silently mirror to SQLite so BOM and quote persistence can reference this scenario
  fetch(`${apiBase}/api/sync/scenarios`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id: saved.id, name: saved.name, description: saved.description, input: saved.input, result: saved.result }),
...
function loadCADDemo(commodity: string): void {
  const result = CAD_AI_DEMOS[commodity];
  if (!result) {
    console.warn('[CAD Demo] No pre-computed result for commodity:', commodity);
    return;
  }
  // Navigate to CAD Analysis section
  showCosting('cad_analysis' as CommodityType);
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      document.getElementById('cad-results')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 100);
  }, 80);
}
...
function addSupplierQuote(): void {
  ...
  // Persist to backend when a scenario has been saved
  if (_activeScenarioId) {
    fetch(`${apiBase}/api/quotes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        scenario_id: _activeScenarioId,
        supplier_name: quote.supplierName,
        unit_price: quote.quotedPriceGBP,
        currency: quote.currency,
        lead_time_weeks: quote.leadTimeDays ? Math.ceil(quote.leadTimeDays / 7) : null,
        notes: quote.notes,
      }),
    }).catch(() => { /* non-critical — in-memory copy still works */ });
  }
}
