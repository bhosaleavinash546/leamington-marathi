...
const SPECIALIST_SYSTEM_PROMPTS: Record<string, string> = {
  ...
  general: 'You are a world-class PCB engineer and electronics cost analyst with 20+ years of experience across multiple industries. You analyse PCB images with exceptional accuracy and provide realistic component pricing based on 2024 UK market data. Return ONLY valid JSON.',
};

// ── Pricing reference table ────────────────────────────────────────────────
const PRICING_TABLE = `COMPONENT PRICING REFERENCE — UK 2024, use these ranges as hard anchors:
passive_0402: resistors £0.003–0.010, caps £0.005–0.060 (X5R/X7R); auto-grade: 3–6× above
passive_0603: resistors £0.005–0.015, caps £0.008–0.120, inductors £0.020–0.150
passive_0805: resistors £0.008–0.025, caps £0.015–0.500, inductors £0.050–1.50
crystal_osc: HC-49 crystal £0.15–0.45; SMD crystal £0.20–0.80; TCXO £1.20–6.00; OCXO £12–60; auto-grade 3×
power_module: DC-DC SIP/DIP module £2.50–12; isolated module £8–35; automotive £20–80
transformer: SMD signal transformer £0.80–3.50; SMD power transformer £2–15; common-mode choke £0.30–2.50
led: SMD indicator 0603/0805 £0.04–0.15; RGB LED £0.12–0.60; high-power LED £0.50–4.00
relay_switch: SMD relay SPDT £0.35–2.00; high-current relay £2–10; tactile switch £0.05–0.50
fuse_tvs: SMD polyfuse £0.08–0.35; SMD fuse £0.05–0.25; TVS diode £0.08–0.50; TVS array £0.25–1.20
ic_soic: logic gate £0.12–0.60; op-amp general £0.25–2.50; op-amp precision £1–8; driver IC £0.30–4.00; LDO regulator £0.20–2.50; comparator £0.15–1.50
ic_qfn: simple MCU (8/32-bit low-end) £0.50–3.50; complex MCU £3–15; PMIC £1.50–12; RF IC £2–20; industrial MCU £3–25
ic_bga: FPGA small £10–60; FPGA large £50–400; SoC/Application CPU £30–250; DDR memory £3–20; automotive SoC £40–300; ADAS processor £100–600
ic_tqfp: MCU 32-bit mid-range £2–12; DSP £5–30; CPLD £3–20; automotive MCU £8–60
connector_smt: 0.5mm FPC/FFC £0.20–0.90; 1.0mm FPC £0.15–0.60; USB-C £0.30–1.50; SMA/RF £0.50–3.00; DF17 board-to-board £1.20–6.00; automotive connector (Kostal/Amphenol) £2–15
through_hole: electrolytic cap (small) £0.12–0.80; electrolytic cap (large) £0.50–5.00; TH connector 2-row £0.30–3.00; power connector £0.80–8.00; TO-220 transistor £0.30–5.00
manual_solder: wire/jumper £0.10–0.50; heat-shrink joint £0.05–0.30`;
...
function buildICPriceHints(markings: string[], _domain: string): string {
  return markings.map(marking => {
    ...
    if (m.includes('STM32')) return `${marking} — STM32 microcontroller — £2.50–12 depending on variant`;
    if (m.includes('TJA')) return `${marking} — NXP TJA CAN/LIN transceiver — £0.80–4.00`;
    if (m.includes('AURIX') || m.includes('TC2') || m.includes('TC3')) return `${marking} — Infineon AURIX MCU — £25–120 automotive grade`;
    if (m.includes('TDA4') || m.includes('TDA2')) return `${marking} — TI TDA4/2 ADAS SoC — £80–350`;
    if (m.includes('S32K') || m.includes('S32G')) return `${marking} — NXP S32 automotive MCU/SoC — £8–90`;
    if (m.includes('NRF') || m.includes('NRF')) return `${marking} — Nordic Semiconductor nRF MCU/SoC — £1.50–8.00`;
    if (m.includes('ESP32') || m.includes('ESP8')) return `${marking} — Espressif ESP32/ESP8266 WiFi SoC — £1.00–4.00`;
    if (m.includes('BCM') || m.includes('LAN')) return `${marking} — Broadcom/Microchip Ethernet IC — £1.50–15`;
    if (m.includes('MAX') || m.includes('LM') || m.includes('TLV')) return `${marking} — Analog/TI op-amp or PMIC — £0.25–8.00`;
...
- unitPriceGBP: use the COMPONENT PRICING REFERENCE above as hard anchors
